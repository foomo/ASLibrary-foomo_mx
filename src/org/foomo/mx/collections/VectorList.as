...
package org.foomo.mx.collections
{
	...
	public class VectorList extends EventDispatcher implements IList, IExternalizable, IPropertyChangeNotifier
	{
		...
		/**
		 *  Construct a new VectorList using the specified array as its source.
		 ...
		 *  source array.
		 *  If no source array was supplied the VectorList will create one internally.
		...
		public function set source(value:*):void
		{
			...
			if (this._source && this._source.length) {
				...
				for (i = 0; i < len; i++) this.stopTrackUpdates(_source[i]);
			}

			this._source  = value ? value : [];

			len = _source.length;
			for (i = 0; i < len; i++) startTrackUpdates(_source[i]);
		...
		public function setItemAt(item:Object, index:int):Object
		{
			...
			if (_dispatchEvents == 0) {
				...
				if (hasPropertyListener) dispatchEvent(updateInfo);
			}
			return oldItem;
		}
		...
		public function removeAll():void
		{
			if (length == 0) return;
			var len:int = length;
			for (var i:int = 0; i < len; i++) this.stopTrackUpdates(source[i]);
		...
		public function toArray():Array
		{
			...
			for (var i:int;i<source.length; i++) ar[i] = source[i];
			return ar;
		}
		...
		{
			return (this.source) ? this.source.toString() : getQualifiedClassName(this);
		}
		...
		private function enableEvents():void
		{
			...
			if (this._dispatchEvents > 0) this._dispatchEvents = 0;
		}
		...
		protected function itemUpdateHandler(event:PropertyChangeEvent):void
		{
			...
			if (this._dispatchEvents == 0 && this.hasEventListener(PropertyChangeEvent.PROPERTY_CHANGE)) {
